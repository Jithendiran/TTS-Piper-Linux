...
void playback()
{
    ...
    int readfd = open("/media/ssd/Project/TTS-Piper-Linux/poc/alsa/poc-l.wav", O_RDONLY);
    if (readfd < 0)
    {
        perror("wavread");
        exit(1);
    ...
    // check is in right state
    while (snd_pcm_state(handle) != SND_PCM_STATE_PREPARED) { print_pcm_state(handle); }

    while (1)
    {
        // check speaker playback is completed
        if (snd_pcm_state(handle) == SND_PCM_STATE_PREPARED || snd_pcm_state(handle) == SND_PCM_STATE_RUNNING)
        {
...
/*
Todo
 isready -> after start, when accept input
 can write -> when accept playback
 is completed -> when playback is completed and speaker is idle
 async
*/
